...
class Generator {
    ...
    isDebug = false;
    statusId = Generator.DONE.status;
    ...
    init() {
        ...
        this.timer.setDebug(this.isDebug);
            ...
            this.addTask(() => {
                ...
                let promise = new Promise(/*async*/(resolve, reject) => {
                    ...
                    this.sleepSetIntervalID = setInterval(() => {
                        ...
                        if (this.statusId === Generator.ON.status) {
        ...
        this.statusId = Generator.INIT.status;
    ...
    generate(date, timerId) {
        ...
        this.addTask(() => {
            let on = Timer.randomNumber(this.isDebug, this.min, this.max) * this.timeout;
            ...
            if (!this.controller.signal.aborted) {
                ...
                Timer.wait(this.timeout, this.controller.signal.aborted, timerId).then((timerId) => {
                    this.timer.invoke(this.indicator);

                    // Watchdog
                    Timer.wait(Generator.TIMEOUT, this.controller.signal.aborted, timerId).then(() => this.timer.prevent(this.indicator)).catch(e => this.subject.notifyAll({ "message": JSON.stringify(e) }, "Generator"));
        ...
        this.statusId = Generator.ON.status;
     ...
     */
        ...
        this.statusId = Generator.DONE.status;
    ...
    terminate(reason) {
        ...
        this.statusId = Generator.OFF.status;
     ...
     */
    status() {
        return Generator.getStepByStatus(this.statusId);
    }
}
...
class Generator {
    ...
    /**
     * Set the max. number of indicators which are ON at the same time.
     * 
     * @param {*} maxNumber 
     * @returns 
     */
    ...
    /**
     * Set the min. time where a indicator must be on.
     * 
     * @param {*} minOnTime 
     * @returns 
     */
    setMinOnTime(minOnTime) {
        //
    ...
    init() {
        ...
        // Set timer listener
        this.timer.setListener(Timer.INVOKE_KEY, (options) => { this.timer.toggle(options.indicator, options.previous, Timer.INVOKE_KEY); });
        this.timer.setListener(Timer.PREVENT_KEY, (options) => { this.timer.toggle(options.indicator, options.previous, Timer.PREVENT_KEY); });

        // Run timer initialization and subscribe observers
        this.timer.init.apply(this.timer, observers);

        // Switch on / off all indicators
        Indicator.INDICATORS.forEach(indicator => {
            this.timer.invoke(indicator).done();

            let timerId = Number.MIN_SAFE_INTEGER;
            Timer.wait(wait, this.controller.signal.aborted, timerId).then(() => {
                this.timer.prevent(indicator).done();
            }).catch(e => this.subject.notifyAll({ "message": JSON.stringify(e) }, "Generator"));
        });
                ...
                let promise = new Promise(async (resolve, reject) => {
                    ...
                    this.sleepSetIntervalID = setInterval(() => {
                        ...
                        if (this.status === Generator.ON.status) {
                            resolve(() => this.generate(Date.now(), Indicator.INDICATORS.length, Number.MIN_SAFE_INTEGER));
                        } else {
                            clearInterval(this.sleepSetIntervalID);
                            reject("Generator: Something went wrong! Used generator might not be configured and initialized properly.");
                        }
    ...
    generate(date, timerId) {
        ...
        this.addTask(() => {
            ...
            if (!this.controller.signal.aborted) {
                ...
                this.timer.invoke(Indicator.M).done();

                Timer.wait(wait, this.controller.signal.aborted, timerId).then((timerId) => {
                    this.generate(Date.now(), timerId);
                    this.timer.prevent(Indicator.M).done();
                }).catch(e => this.subject.notifyAll({ "message": JSON.stringify(e) }, "Generator"));
                ...
                this.subject.clearAll(Indicator.INDICATORS[0], "Generator");
            }
        });
