...
class Timer {
    ...
    /**
     * Round number. See 'https://www.delftstack.com/de/howto/javascript/javascript-round-to-2-decimal-places/'
     * 
     * @param {*} x 
     * @param {*} y 
     * @returns 
    ...
    static wait(timeout, aborted, timerId) {
        ...
        if (timeout < 0) {
            ...
            let promise = new Promise((resolve, reject) => {
                if (!aborted) {
                    timerId = window.setTimeout(() => { resolve(timerId); }, (timeout || Timer.TIMEOUT) * 1000);
                } else {
                    clearTimeout(timerId);
                    reject("Wait: Terminate timer task. Cleared timeout with ID '" + timerId + "'. Timeout was set to '" + timeout + "'.");
                }
        ...
        this.addTask(() => {
            ...
            if (timeout < 0) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    this.sleepSetTimeoutID = window.setTimeout(() => {
                        // this.subject.notifyAll({ "message": `End sleeping for '${timeout}' seconds.` }, "Timer");
