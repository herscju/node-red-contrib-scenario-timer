...
module.exports = function (RED) {
    ...
    /**
     * Main function to run in Node-Red environments.
     * 
     * @param {*} config 
     */
    function ScenarioTimerNode(config) {
        ...
        // First indicator is used as watchdog indicator
        ...
        node.on('input', (msg) => {
            ...
            if (!isNaN(msg.payload)) {
                ...
                switch (msg.action) {
                    case Generator.GENERATE:
                        status = generator.setTimeout(node.timeout).setMaxNumber(node.maxNumber).setMinOnTime(node.minOnTime).init(ledBulb, nodeRed, trigger).generate(Date.now(), Number.MIN_SAFE_INTEGER).state();
